...
# --- FIX: Changed import name to match the generated model file name ---
from SNN_Unet import SNNUNetJelly as Model
...
if __name__ == '__main__':
    ...
    train_dl = DataLoader(
        train_ds,
        batch_size=16, 			# smaller batch to start
        shuffle=True,
        num_workers=0, 			# <-- key
    ...
    # IMPORTANT: T=20 is used here. This value must be used to create the time dimension.
    model = Model(in_ch=4, out_ch=1, T=20, tau=10).to(device)
    ...
    for epoch in range(1, epochs+1):
        ...
        for imgs, masks in pbar:
            ...
            # --- SNN FIX: EXPAND 4D INPUT [B, C, H, W] TO 5D [T, B, C, H, W] ---
            # The model requires the time dimension first, containing the static image replicated T times.
            imgs_5d = imgs.unsqueeze(0).repeat(model.T, 1, 1, 1, 1)
            
            logits = model(imgs_5d) # Pass the 5D tensor
